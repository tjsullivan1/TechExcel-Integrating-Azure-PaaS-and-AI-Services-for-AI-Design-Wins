...
def make_cosmos_db_vector_search_request(
    query_embedding, max_results=5, minimum_similarity_score=0.5
):
    ...
    results = container.query_items(
        query=f"""
            ...
            """,
        parameters=[{"name": "@request_vector", "value": query_embedding}],
...
def main():
    ...
    minimum_similarity_score = st.slider(
        "Minimum Similarity Score:", min_value=0.0, max_value=1.0, value=0.5, step=0.01
    )
    if st.button("Submit"):
        with st.spinner("Searching transcripts..."):
            if query:
                query_embedding = (
                    make_azure_openai_embedding_request(query).data[0].embedding
                )
                response = make_cosmos_db_vector_search_request(
                    query_embedding, max_results, minimum_similarity_score
                )
